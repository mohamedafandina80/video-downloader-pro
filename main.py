...
# 👑 حط بيانات البروكسي بتاعك هنا (هتستبدل البيانات دي باللي هتاخدها من الموقع)
# الصيغة: http://username:password@IP:PORT
# 👑 رابط البروكسي اللي هيخفي السيرفر بتاعك
PROXY_URL = "http://zxkmncca:twmx50gdx5gi@31.59.20.176:6754"
...
YDL_OPTS = {
    ...
    'extract_flat': False,
    'proxy': PROXY_URL, # 👑 حقن البروكسي لفك الحظر
}
...
async def download(url: str, format_id: str, is_audio: bool = False, needs_merge: str = "false"):
    ...
    def stream_data():
        if not needs_merge_bool and not is_audio:
            ...
            command = [
                'yt-dlp',
                '--proxy', PROXY_URL, # 👑 حقن البروكسي في التحميل
                '-f', target_format,
                '--quiet', '--no-warnings',
                '-o', '-', 
                url
            ...
            with yt_dlp.YoutubeDL({'quiet': True, 'no_warnings': True, 'proxy': PROXY_URL}) as ydl:
                info = ydl.extract_info(url, download=False)
